<template>
  <div class="users-big-modal">
    <ButtonCloseModal />
    <p class="head-text">Добавить роли</p>
    ...
    methods: {
      ...
      async submitAddRoles() {
       ...
       for (const role of this.selectedRoles)  {
         formData.append(`r_ids[]`, role.id);
       }

        for (const user of this.userIds)  {
          formData.append(`u_ids[]`, user.id);
        }

        formData.append(`what`, 'for-users');

        const {slug} = this.$store.state.user.info.actual_role;

        await this.axios.post(`${process.env.VUE_APP_API_URL}${slug}/roles`, formData,
          {
